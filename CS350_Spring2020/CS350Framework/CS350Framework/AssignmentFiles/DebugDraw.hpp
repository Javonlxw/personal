...
public:
  ...
  //My own helper functions to fill up the vector of LineSegments in a DebugShape obj
  void CalculateOrthogonalBasis(const Vector3& u, Vector3& v, Vector3& w);
  void CalculateRay(const Ray& ray, float t, std::vector<LineSegment>& shape_lines);
  void CalculateQuad(const Vector3& p0, const Vector3& p1, const Vector3& p2, const Vector3& p3, std::vector<LineSegment>& shape_lines);
  void GetHorizontalDisc(const Sphere& sphere, Vector3& HD_center, float& HD_radius);
  void CalculateDisc(const Vector3& center, const float& radius, const Vector3& axis, std::vector<LineSegment>& shape_lines);
